...
def extraer_api(url):
    ...
    if response.status_code == 200:
        ...
        df.to_csv("datasets/pob_total_bc.csv")

        print("Extracción de población total realizada con éxito.")

    else:
        print(f"Error {response.status_code} en la API")

def ejecutar_total():
    url = "http://www.economia.gob.mx/datamexico/api/data?State=2&cube=inegi_enoe&drilldowns=State,Quarter&measures=Workforce&locale=es&parents=false"
    extraer_api(url)

#Descomentar en caso de querer hacer pruebas individuales
#Para las pruebas individuales en necesario cambiar el guardado a df.to_csv("../datasets/pob_total_bc.csv")
