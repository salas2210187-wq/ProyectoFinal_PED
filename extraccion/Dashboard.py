...
def charts_salario_por_genero():
    ...
    if "Salario_Promedio_Mensual" in df.columns and "Sexo" in df.columns:
        ...
        # heatmap pivot año/periodo vs género
        try:
...
def charts_composicion_formalidad():
    ...
    if df is None:
        return None, None, None
    # donut último perido por tipo de empleo
    if "Tipo_Empleo" in df.columns and "Cantidad_Trabajadores" in df.columns:
        ...
        # stacked bars evolución por periodo
        try:
...
def sidebar_controls(global_state):
    ...
    year_min = None
    year_max = None
    # construir una unión de años disponibles en todas las vistas
    all_years = set()
    for v in VIEWS:
        df = load_view_df(v)
        if df is not None:
